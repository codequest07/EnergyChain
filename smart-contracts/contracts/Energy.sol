...
contract Energy is ReentrancyGuard {
    ...
    event ProducerWithdrawal(address withdrawer, uint amount);
    ...
    function purchaseEnergyCredits(
        address producer,
        uint creditAmount
        ...
        if (IERC20(energyToken).balanceOf(msg.sender) < totalCost)
            revert InsufficientTokenBalance();

        // Transfer the tokens from the buyer to the contract (instead of the producer directly)
        bool success = IERC20(energyToken).transferFrom(
            msg.sender,
            address(this),
            totalCost
        );
    ...
    function withdraw() external nonReentrant {
        ...
        bool success = IERC20(energyToken).transfer(
            msg.sender,
            amountToWithdraw
        );
